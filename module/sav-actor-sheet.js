...
export class SaVActorSheet extends SaVSheet {
  ...
  static get defaultOptions() {
  	  ...
  	  template: "systems/ziggurat/templates/actor-sheet.html",
      width: 800,
      height: 970,
  ...
  async getData(options) {
    ...
    let ship_actors = this.actor.getFlag("ziggurat", "ship") || [];
    let shipActor = game.actors.get( ship_actors[0]?.id );
    // If assigned ship no longer exists, remove from flags
    if( shipActor === undefined ) { this.actor.setFlag("ziggurat", "ship", ""); }
  ...
  /** @override */
    ...
    html.find('.item-delete').click( async (ev) => {
      ...
      const element = $(ev.currentTarget).parents(".item");
      await this.actor.setFlag("ziggurat", element.data("itemType"), "");
      element.slideUp(200, () => this.render(false));
	  });
