...
export class SaVItem extends Item {
   ...
   async _preCreate( data, options, user ) {
     ...
     if( user.id === game.user.id ) {
      ...
      let stars = await SaVHelpers.getFiles("systems/ziggurat/styles/assets/stars/star*", ".webp", true);
      let random = Math.floor( Math.random() * stars.length ) + 1;
      this.updateSource( { img: stars[random] } );
    }
		if ( this.type === "planet" ) {
      let planets = await SaVHelpers.getFiles("systems/ziggurat/styles/assets/planets/planet*", ".webp", true);
   ...
   async _onCreate( data, options, userId ) {
     ...
     if( userId === game.user.id ) {
       ...
       if( ( actor?.documentName === "Actor" ) && ( actor?.isOwner ) ) {
         ...
         if( ( ( data.type === "class" ) || ( data.type === "crew_type" ) ) && ( ( actor.img.slice( 0, 46 ) === "systems/ziggurat/styles/assets/icons/" ) || ( actor.img === "icons/svg/mystery-man.svg" ) ) ) {
           const icon = data.img;
           const icon_update = {
             img: icon,
             token: {
  ...
  async sendToChat() {
    ...
    const html = await renderTemplate("systems/ziggurat/templates/items/chat-item.html", itemData);
    const chatData = {
      user: game.userId,
      content: html,
    };
